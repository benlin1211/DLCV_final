...
class Res16UNet34C(Res16UNet34): # we run it
...
class Res16UNet34C_TransformerEncoder3(ResNetBase): # we run it
    # Follow Res16UNet34C 
    BLOCK = BasicBlock
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
    LAYERS = (2, 3, 4, 6, 2, 2, 2, 2)
    ...
    CONV_TYPE = ConvType.SPATIAL_HYPERCUBE_TEMPORAL_HYPERCROSS
    # Add
    TRANSFORMER_DIM = 96 # same as unet output
    N_HEAD = 8
    NUM_LAYERS = 1
    MAX_TGT_LEN = 100000
    ...
    # Once data is processed, call clear to reset the model before calling initialize_coords
    def __init__(self, in_channels, out_channels, config, D=3, **kwargs):
        super(Res16UNet34C_TransformerEncoder3, self).__init__(in_channels, out_channels, config, D)
    ...
    def forward(self, x):
        ...
        # Add: forward to transformer.
        print("feat",out.shape)

        # epmty = torch.empty(self.MAX_TGT_LEN, self.config.batch_size, self.TRANSFORMER_DIM)
        pcd_size = out.shape[0]
        curr_sample_size = self.MAX_TGT_LEN 
        if pcd_size <= curr_sample_size:
            ...
            mask_idx = torch.ones(curr_sample_size,
                                dtype=torch.bool)

            idx[:pcd_size] = torch.arange(pcd_size)

            mask_idx[:pcd_size] = False  # attend to first points
        else:
            # we have more points in pcd as we like to sample
            # take a subset (no padding or masking needed)
            idx = torch.randperm(pcd_size)[:curr_sample_size]
            mask_idx = torch.zeros(curr_sample_size,
                                  dtype=torch.bool)  # attend to all
        
        # 有 padding 
        out = self.transformer_encoder(out)

        fin = self.final(out)
        print("output:", fin.shape)
        return fin, out
